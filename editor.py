...
.lp-frame.is-changed{outline:2px solid #E8B500;outline-offset:6px;background:rgba(255,214,0,.16)}
.lp-frame.is-changed:focus-within{outline-color:#EE4729}
...
(function () {
  ...
  function markAll() {
    ...
    document.querySelectorAll('x-t[data-k]').forEach(function (n) {
      ...
      frameOf(n).classList.toggle('is-changed', now !== was);
    });
    var cnt = document.querySelectorAll('.lp-frame.is-changed').length;
  ...
  bar.innerHTML =
    ...
    '<div class="lp-row">' +
      '<span class="lp-count">изменений нет</span><span class="sp"></span>' +
      '<button data-act="hidden">Скрытые строки (' + HIDDEN.length + ')</button>' +
      '<button data-act="one">Скачать текущий язык</button>' +
      '<button data-act="all" class="primary">Скачать все языки</button>' +
...
def build_page(page_html, lang, data, tpl, all_locales, lang_names):
