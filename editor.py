...
(function () {
  ...
  function token(force) {
    var t = localStorage.getItem('ketology-gh-token');
    if (!t || force) {
      t = (prompt('GitHub-токен с правом записи в ' + REPOS[0].repo +
                  '\n(сохранится только в этом браузере)') || '').trim();
      if (!t) return null;
  ...
  function publish(btn) {
    ...
    chain.then(function () {
      ...
      btn.disabled = false;
      if (String(err).indexOf('401') > -1 || String(err).indexOf('Bad credentials') > -1) {
        localStorage.removeItem('ketology-gh-token');
        alert('Токен не подошёл. Нажмите «Сохранить в проект» ещё раз и введите новый.');
      } else {
